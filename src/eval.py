...
class Evaluator:
    ...
    def __init__(self, device, ex, benchmark, disable_tqdm=False):
        self.device = device
        self.ex = ex
    ...
    def generate_tasks(self, loaders_dic, extracted_features_dic, model_path, shot, model_tag, used_set, meta_test_iter):
        ...
        tasks_dics = []
        for _ in warp_tqdm(range(meta_test_iter), self.disable_tqdm):
    ...
    @eval_ingredient.capture
    def run_full_evaluation(self, model, model_path, model_tag, shots, method, meta_val_way, 
            init_mode, callback):
    ...
    def run_task(self, task_dic, train_mean, model, meta_val_way, shot, init_mode, callback):
        ...
        tim_builder.init_weights(support=support, y_s=y_s, query=query, y_q=y_q, 
                meta_val_way=meta_val_way, shot=shot, init_mode=init_mode)
    ...
    def get_tim_builder(self, model, method):
        ...
        tim_info = {'model': model, 'disable_tqdm': self.disable_tqdm, 'benchmark': self.benchmark}
        if method == 'tim_adm':
    ...
    def get_loaders(self, used_set, target_data_path, target_split_dir, meta_test_iter, meta_val_way,
        ...
        train_loader = get_dataloader('train', **loader_info).get()
        loaders_dic['train_loader'] = train_loader
        try:  # In case there are predefined support-query files (for iNat only)
            support_loader = get_dataloader('support', **loader_info).get()
            query_loader = get_dataloader('query', **loader_info).get()
            loaders_dic.update({'query': query_loader, 'support': support_loader})
        except:  # For all other datasets
            test_loader = get_dataloader(used_set, **loader_info).get()
        ...
        for shot in shots:
            ...
            episodic_test_loaders[shot] = get_dataloader(used_set, **loader_info).get()
        loaders_dic.update({'episodic_test': episodic_test_loaders})
        return loaders_dic

    @eval_ingredient.capture
    def extract_features(self, model, model_path, model_tag, used_set, loaders_dic, overwrite, benchmark):

        if benchmark:
            with torch.no_grad():
                for i, (inputs, _, _) in enumerate(loaders_dic['train_loader']):
                    inp = inputs
                inp = inp.to(self.device)
 
                stmt = """
outp, _ = model(inp, True, True)
...
torch.cuda.synchronize()
                """
                globals()['model'], globals()['inp'] = model, inp
                torch.cuda.synchronize()
                ...
                print('fe: {}'.format(total_time / 100))
        ...
        with torch.no_grad():
            out_mean = []
            for i, (inputs, _, _) in enumerate(warp_tqdm(loaders_dic['train_loader'], self.disable_tqdm)):
            ...
            if 'test' in loaders_dic:
                ...
                all_features = []
                all_labels = []
                for i, (inputs, labels, _) in enumerate(warp_tqdm(loaders_dic['test'], self.disable_tqdm)):
                    inputs = inputs.to(self.device)
                    outputs, _ = model(inputs, True)
                ...
                for i, (inputs, labels, _) in enumerate(warp_tqdm(loaders_dic['support'], self.disable_tqdm)):
                ...
                for i, (inputs, labels, _) in enumerate(warp_tqdm(loaders_dic['query'], self.disable_tqdm)):
                    inputs, labels = inputs.to(self.device), labels.to(self.device)
                    outputs, _ = model(inputs, True)
                    outputs = outputs.cpu()
                    for out, label in zip(outputs, labels):
